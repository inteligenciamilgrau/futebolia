...
function setupSocket() {
    const apiPath = window.PLATFORM_GAME_CONFIG?.apiBasePath || '';
    socket = io(apiPath);

    socket.on('init', (state) => {
        // Limpar jogadores antigos antes de iniciar nova sessão
        players.forEach(p => p.destroy(scene));
        players.clear();
    ...
    socket.on('state-update', (state) => {
        ...
        // Remover jogadores que não estão no estado (fantasmas de outros modos)
        ...
        state.players.forEach(pData => {
            ...
            if (p) {
                ...
                // Se o jogador surgiu no meio da partida (novo join)
                players.set(pData.id, new Player(pData, scene, mats));
            }
        });
...
// Expor cena para depuração ou limpeza externa se necessário
